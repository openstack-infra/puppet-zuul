...
class zuul::executor (
  ...
  exec { 'install-ara-safely':
    command => '/usr/bin/pip3 install --upgrade --upgrade-strategy=only-if-needed ara',
    ...
    # ansible).
    onlyif  => '/bin/bash -c -- test $(/usr/bin/pip3 list --format columns | sed -ne \'s/^ara\s\+\(.*\)$/\1/p\') != $(/usr/bin/pip3 search \'ara$\' | sed -ne \'s/^ara (\(.*\)).*$/\1/p\')',
