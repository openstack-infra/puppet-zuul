...
define zuul::status_backups (
  ...
  if tenant_name and tenant_name != "" {
    ...
    cron { "zuul_scheduler_status_backup-$name":
      user    => 'root',
      command => "timeout -k 5 10 curl ${status} -o /var/lib/zuul/backup/$name_status_$(date +\\%s).json 2>/dev/null",
    ...
    cron { "zuul_scheduler_status_prune-$name":
      user    => 'root',
      minute  => '0',
      command => "flock -n /var/run/$name_status_prune.lock ls -dt -1 /var/lib/zuul/backup/$name_* |sed -e "1,120d" |xargs rm -f",
